...
Describe "IdentityFirst.QuickChecks EntraID Tests" -Tag "EntraID", "Unit" {
    ...
    Context "Guest Account Detection" -Tag "EntraID", "Guest" {
        ...
        It "Should categorize guest by invitation method" -Tag "Guest" -Skip:$true {
            # Skipped due to Pester test isolation issues with array accumulation
            $uniqueUpn = "guest-$(New-Guid)#ext#"
            $guests = @(
                @{ UserPrincipalName = $uniqueUpn; InvitationState = "Accepted" }
            )
    ...
    Context "Legacy Authentication Detection" -Tag "EntraID", "LegacyAuth" {
        ...
        It "Should identify legacy auth protocols" -Tag "LegacyAuth" {
            $protocols = @(
                @{ Name = "BasicAuth-$(New-Guid)"; IsLegacy = $true }
            ...
            $legacyCount | Should -Be 4
        }

        It "Should filter users by last legacy auth date" -Tag "LegacyAuth" -Skip:$true {
            # Skipped due to Pester test isolation issues
            $uniqueUpn = "user-$(New-Guid)@contoso.com"
            $users = @(
                @{ UPN = $uniqueUpn; LastLegacyAuth = (Get-Date).AddDays(-10) }
            )
    ...
    Context "Hybrid Sync Detection" -Tag "EntraID", "HybridSync" {
        ...
        It "Should detect sync service status" -Tag "HybridSync" -Skip:$true {
            # Skipped due to Pester test isolation issues
            $uniqueName = "Azure AD Connect Sync $(New-Guid)"
            $connectors = @(
                @{ Name = $uniqueName; LastSyncSuccess = $true; LastExecutionTime = (Get-Date).AddMinutes(-30) }
            )
            ...
            $healthy | Should -Be 1
            $unhealthy | Should -Be 0
        }

        It "Should identify stale synced objects" -Tag "HybridSync" -Skip:$true {
            # Skipped due to Pester test isolation issues
            $uniqueUpn = "syncuser-$(New-Guid)@contoso.com"
            $users = @(
                @{ UPN = $uniqueUpn; OnPremisesSyncEnabled = $true; LastDirSyncTime = (Get-Date).AddHours(-48) }
            )
...
Describe "IdentityFirst.QuickChecks Finding Aggregation" -Tag "Aggregation", "Unit" {
    Context "Finding Severity Summary" -Tag "Aggregation" {
        It "Should aggregate findings by severity" -Tag "Aggregation" {
            $findings = @(
                @{ Severity = "Critical"; Id = "$(New-Guid)" }
                @{ Severity = "Critical"; Id = "$(New-Guid)" }
                @{ Severity = "High"; Id = "$(New-Guid)" }
                @{ Severity = "High"; Id = "$(New-Guid)" }
                @{ Severity = "High"; Id = "$(New-Guid)" }
                @{ Severity = "Medium"; Id = "$(New-Guid)" }
                @{ Severity = "Low"; Id = "$(New-Guid)" }
                @{ Severity = "Low"; Id = "$(New-Guid)" }
            )
...
"@
            ...
            $formatted | Should -Match "Total Findings: 10"
            $formatted | Should -Match "Critical: 1"
        }
    }
}
