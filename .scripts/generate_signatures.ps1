...
# Generate signatures.json for all .ps1 and .psm1 files under repository
$repoRoot = Split-Path -Parent $MyInvocation.MyCommand.Path | Resolve-Path | ForEach-Object { Join-Path $_ '..' }
Set-Location $repoRoot

$results = @()
Get-ChildItem -Path . -Include *.ps1,*.psm1 -Recurse -File | ForEach-Object {
    $path = $_.FullName
    try {
    ...
    $obj = [PSCustomObject]@{
        ...
        Signer = if ($s -and $s.SignerCertificate) { $s.SignerCertificate.Subject } else { $null }
        TimeStamp = if ($s -and $s.TimeStamperCertificate) { $s.TimeStamperCertificate.NotBefore } else { $null }
    }
    $results += $obj
}

$out = ConvertTo-Json $results -Depth 4
$out | Set-Content -Path (Join-Path $repoRoot 'signatures.json') -Encoding UTF8
Write-Output "Wrote signatures.json with $($results.Count) entries"
# SIG # Begin signature block
# MIIf3QYJKoZIhvcNAQcCoIIfzjCCH8oCAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDsWxcixyEgCMOX
# ru44KKOIOTqFMFlxzGw/RzLS8sh3eKCCGNwwggWNMIIEdaADAgECAhAOmxiO+dAt
...
# AgEVMC8GCSqGSIb3DQEJBDEiBCCA7F2Q2LY8GxRLo5Hmjbj8HMQNj/svW1TUnXQY
# Ikd4dDANBgkqhkiG9w0BAQEFAASCAgDMPqSJMlXuXwMf/MDHjOw+5yEIExqmdl5i
# JOAZz9chULMrG8Sx2st74pzoLksrRE8ILTZ6lLPlKF+Skq89x4nN38qOazC8kRpR
# 1OAaYc8e0il4vnEE3cmqCsY7WSWFDLwN0HCFCDGrjaQ2Vsf5553ZEO1nFEi2yUPN
# mZVk4i92NPbabo4txrllO2jRlf4uXxCBo8eJ8OBzduOUb+pbaE7LdyxZ5CVI1I4Q
# BlGwCjLEaQN176prP1eb3czkPGvbkRf1s676nKaZVK7TksItn7a2cxwXscnsqCHm
# 6EJg9xTrGW01HCt7Tb9dZx/TQmCKNOizXwvu1XhmbnelTEWNqufnCEintxn0pN+1
# dYzo13vyX09an7Du6A31xWuWh7RBVPlWH1R0iHaiqURAzSavM+KQ2Hp4dJ6Dr/8u
# rlQlM2MbKCJV/hCi4C/CBuEzbFCVWvZIHgOKEvM64Pt7r3pWv7W7MOBYymCjdPOy
# ZCwfIEYgkpzN/r4bmIQu1RT7605OCuiqOOuOUXu1e+U+wB03+QnDA+pENaMTclX2
# TRfSl97wXk2yl04kEyagsgij7yD/jgaPXSsUvU3qOLRAa8WogBHHzfYzQsB/rV8V
# xYFqNMS8wqIKwJYT5Xb2VEGpNusWUmPTOzav0AArk78Csv7xUY8DnFClZunLqeub
# 0oHvcAPCZKGCAyYwggMiBgkqhkiG9w0BCQYxggMTMIIDDwIBATB9MGkxCzAJBgNV
...
# CSqGSIb3DQEJBDEiBCD4zuszGBpz7IPtgYmLkPDi6y5VC4fyNwAtYYdBhqloSDAN
# BgkqhkiG9w0BAQEFAASCAgCjD4MkmsYfwTSIbBDuA8/UFThE59zm3Vyn8lqUCDJ5
# 4l3RybpHJvTfl7z5V2zd96urHu7pKq9/VvbTGp8wpKSsDAfxWaQjS8U9H63u1Rhq
# s8Yo1eZOyT8niQf6IAZmT0IvWhykKGGzNES0/bUCRHFTnXxnZnzP58Lc2myrGnlG
# NjCHede39W7IbqhuPWxpGR0a2lN9XYaCIRBv15F82UrThbGEHvAJokzVLZxDLM2n
# uvb4NjVswshM10U+4ovql+dhVWHPUu2Y2BdGsl/F+VTH3YOK3LEWifus1RyteF/Z
# EW+qMNepa1R+yQYSPMhEvFZowR6DlAigCY1qjN8wztyPRQxK+rBPfsiyn0AJYVZP
# i7ZFR7YPgZTevGXZqe7Uzvy6gYZTJHng+HiDMPX+Y2H5OYbt3m0Ss4LQMEywm3Pe
# fAiqZt32ml0FdsON7Iryf5K8qswf39pX/UuC2QN8WlsDYeJzZExCJ8RcTPJdHds6
# YSeOMRgk/nbARqQ/eXW4iApEFyd42kf5FapG7l6w0I98jiYAUtafXKq12TVRU1yK
# nhoS1xtpAQjOea5Yhe7ghyYReWciAgICcLrxl528mDGgzXS7Zca74BPhnK0Hmwlw
# A5xmjAgyGvHmD8WqHnzbMgnRgx5VBfuMM1CS6TlHg3y5J+VEDRM80PHt4g1Y9WXb
# 3g==
