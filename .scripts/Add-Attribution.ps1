# ============================================================================
# Add-IdentityFirstAttribution.ps1
# Adds attribution header to all PowerShell scripts
# ============================================================================

param(
    [Parameter(Mandatory = $false)]
    [string]$Author = "Mark Ahearne",
    
    [Parameter(Mandatory = $false)]
    [string]$Email = "mark.ahearne@identityfirst.net",
    
    [Parameter(Mandatory = $false)]
    [string]$Company = "IdentityFirst Ltd",
...
$psdFiles = Get-ChildItem -Path $Path -Recurse -Filter "*.psd1" -ErrorAction SilentlyContinue

$allFiles = $psFiles + $psdFiles
...
foreach ($file in $allFiles) {
    try {
        ...
        if ($content -match "Author.*Mark Ahearne") {
            Write-Verbose "Skipped (already attributed): $($file.FullName)"
            continue
        }
        
        # Remove existing ATTRIBUTION or AUTHOR header if present
        $newContent = @()
        $skipNext = $false
        ...
        for ($i = 0; $i -lt $content.Count; $i++) {
            ...
            if ($line -match "^#\s*ATTRIBUTION" -or $line -match "^#\s*AUTHOR" -or $line -match "^#\s*=+$") {
                $skipNext = $true
                continue
            }
        ...
        $finalContent = $header.Trim() + "`n" + ($newContent -join "`n")
        
        if (-not $WhatIf) {
            $finalContent | Set-Content -Path $file.FullName -Encoding UTF8 -ErrorAction Stop
        }
        
        Write-Host "Attributed: $($file.FullName)" -ForegroundColor Green
        $count++
    }
    catch {
...
Write-Host ""
Write-Host "Total files attributed: $count" -ForegroundColor Cyan

# SIG # Begin signature block
# MIIf3QYJKoZIhvcNAQcCoIIfzjCCH8oCAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBqkgJJNkZyS/Pr
# 4Zy5X5cirwkjR6Wey0Ebdova9OrxkaCCGNwwggWNMIIEdaADAgECAhAOmxiO+dAt
...
# AgEVMC8GCSqGSIb3DQEJBDEiBCBvPFXZAXDW7XK3cJnFoFTjaYEom+egvfNLOloA
# rCG59zANBgkqhkiG9w0BAQEFAASCAgAnD1+lS0MWWUJ3F8psE6N88npKfE0oMvgF
# kkF/uLoTipWQ3l2DAfZjX2OLMwRUwZca7IYMSYRkeyDX4dd+4wLSUu/NKloJDVQp
# ojU2IvN3R/zwc2WOA0gnmQMaEYEow2/CVJ+g9ExjG+wGJ7d8Q5OhcFMo8FxV2s/B
# dKYlLe/dTUcH4UahMXZ/0Iu/XpdNjOefZ1MFEnNfYbHExn8TLlo8nFRTc9oD7LFX
# jyG3cTGmsb6RMw5gtc8Kiv1CzqxyNA31OoiI8e0LAEKAzCcezCgyQfWHStqiMz9I
