...
# Rule metadata
$ruleName = 'PSAvoidStateChangingInReadOnlyTools'
$severity = 'Warning'

# Detect function definitions that use state-changing verbs
$funcs = $ast.FindAll({ param($n) $n -is [System.Management.Automation.Language.FunctionDefinitionAst] }, $true)
foreach($f in $funcs){
    $fname = $f.Name
    if($fname -match '^(New|Set|Remove|Update)-' -or $fname -match '^(Add|Clear|Install|Uninstall|Enable|Disable)-'){
        $findings += [PSCustomObject]@{
            File = (Resolve-Path -Path $Path).ProviderPath
            RuleName = $ruleName
            Severity = $severity
            Message = "Function '$fname' uses a state-changing verb."
            Line = $f.Extent.StartLineNumber
            Column = $f.Extent.StartColumnNumber
        }
    }
}

# Detect command invocations that call state-changing cmdlets
$cmds = $ast.FindAll({ param($n) $n -is [System.Management.Automation.Language.CommandAst] }, $true)
foreach($c in $cmds){
    $cmdName = $c.GetCommandName()
    if($cmdName){
        if($cmdName -match '^(New|Set|Remove|Update|Add|Clear|Install|Uninstall|Enable|Disable)-' ){
            $findings += [PSCustomObject]@{
                File = (Resolve-Path -Path $Path).ProviderPath
                RuleName = $ruleName
                Severity = $severity
                Message = "Invokes state-changing command '$cmdName'."
                Line = $c.Extent.StartLineNumber
                Column = $c.Extent.StartColumnNumber
            }
        }
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCHwZs8EEVqi3uN
# 9zaWm4CcFtoKVS8J7o4y6AHLfjYptKCCGNwwggWNMIIEdaADAgECAhAOmxiO+dAt
...
# AgEVMC8GCSqGSIb3DQEJBDEiBCAWJToq9B1tPbhs7tl86sOWwqpdA2Um9zT9R1+G
# OyE5uTANBgkqhkiG9w0BAQEFAASCAgBkiVIErYDBjKuL80bMwGRTn0kI32lx3M91
# BIVzPwWexF2mlXQSkbibmpbwP4HPMPHN/ga+eqqKCfyP/zWVEqrz6bnyPNMxTq2A
# xCRsXIvX4OddTF4axlrizO1cZmbvIqS2aFjrjrWlT1PGMph70IF2iJWQLScZok/7
# bq0W5XhtQZlPGRwkNfvNkeXfOdV7MI7WqEYFIG5TeCilhTgRn/wqkLnVaU8pYioA
# 5pJ/Omi50iM8zQ8CKQlIfsS3qCa5ewaU4j0f4qphob4Nrj4Vkj6Fin+aEmLt6UCT
# 0uyu3zIz/8iKl40IuciCFdL+6mg/wwKzX00DBUvlPe+BPpFf7veDzbJndVJKkSfn
# 9kWpiycQKZ2Ss6F/lhobq4b9AOpVohIcBOzRHvjN+PYzYzmB59qRMhXEQFVoaqag
# gHwoFvAje1WcGADUk0ufQEGumNVxN7T3IahZUh22rAerwQhnpRm4AHfKwuQqUEQk
# YeRbnVwczm/uGUphEs/GIMLYh+9NQI2dwMlgipiWy9eN5H64DuE96ncl3ZGT1bRl
# fOS1VdIJgwydnhXrRsNDNsk2+jsmSFnCbAjEWI40HaiIFYuSMN4CoW9Zjq41ag6z
# hrnp0UTDnGXBfMBVAZsCRwGKIFunTieEE5lM7hdfclS68zxcbpRGIspl81b5iNxT
# CvGQQCHFNKGCAyYwggMiBgkqhkiG9w0BCQYxggMTMIIDDwIBATB9MGkxCzAJBgNV
...
# CSqGSIb3DQEJBDEiBCDpTaaTwl3MbeIZ6pwW6uix6e0dxPHUsApHQj1IdN+nUDAN
# BgkqhkiG9w0BAQEFAASCAgCsmG9ZHnUtQeI+ujJWMs7OVDS77lYdqiRQInOfUci6
# 5hLT3BVTu8mV3CVsUvCXL2iBib4+y74/Rc4yGWl3KYkcN8gQdChYRSYsbdIb7Apc
# w/5EGWXVWI7w8pYPfxcGTmPUIhITifnzlU/hsmEXOYCzMiAQn1A05T4PeGzicZjr
# c4Rox19pnwlH+9yijf7eWKFoowW2FYHo2ATx3SK0w4HdRzCU/5Q418y7SkY+F4h1
# dGbPvCK8By0izGvBY7XVsntKPPYD2UuSVHc71rNUNt74FfoHu1vQNHuYVbVc66gA
# OJm7BdZLlvZbyeOaQHoDTLxvJaYXsa57kQaa+A/RvkiLPqIwUy0vtEhMlENX25Kf
# y+QUasw3qFbP5zJuP/gMA5CjnvvvYBHycB8G1tmhmZ+G3Nxn1XQkyLTOd4qxf9Zx
# dqJvPR5jv5PkIwV794zANEUFC6HoXmkRG1gJstBFNnjZc2KEcwsgpV8SD9Jk0zJW
# F2GVdzpox55UHMwf1Ivg4UXT1ZSPeAqLLqEpTKtOTGwKZgcaYILQ6fjjkW5q015k
# 7SAyO8fajG01HOstsC9/prN8zQ5Eozjro6IUlN5aKCL2EUxfYobv/0S7mXWmEwf3
# WYLCjDtbfVhNPPOdnhAkmv5YkP9vbxzVHU4WQkik/nCpJzCen95BdsFtLrgu5At1
# TQ==
