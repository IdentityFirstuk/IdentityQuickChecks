...
# ============================================================================
try { Import-Module PSScriptAnalyzer -ErrorAction Stop } catch { Write-Output 'PSScriptAnalyzer not available; please install module.' }

$files = @('Module\IdentityFirst.QuickChecks.psm1','Security\IdentityFirst.Security.psm1','Shared\ReportFormatter.psm1')
$results = @()
foreach ($f in $files) {
    if (Test-Path $f) {
        try {
            $r = Invoke-ScriptAnalyzer -Path $f -Recurse -Severity Warning,Error -ErrorAction SilentlyContinue
            if ($r) {
                foreach ($issue in $r) {
            ...
            Write-Output ("Analyzer error on {0}: {1}" -f $f, $_.Exception.Message)
        }
    } else {
...
$results | ConvertTo-Json -Depth 6 | Out-File pssa-modules-report.json -Encoding utf8
Write-Output 'pssa-modules-report.json written
# SIG # Begin signature block
# MIIf3QYJKoZIhvcNAQcCoIIfzjCCH8oCAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAP31PxtTX6o3FQ
# ZW36R1cKiTSUvoiP4/EMcMdnYuiZiaCCGNwwggWNMIIEdaADAgECAhAOmxiO+dAt
...
# AgEVMC8GCSqGSIb3DQEJBDEiBCCj1lCm2HE187RvnIur0DPku1HPFVVfNy3cqBBd
# ya7llTANBgkqhkiG9w0BAQEFAASCAgBWTECaDftifnFe6Ch/shsaDjR2t4H81OEY
# 1SFkG0nC0ThADFMmjBfgvyCe7TjS2SPuXNlGrC3JvtaplqeCV02DN61uTOVjjcr1
# 06wPbbVua/fm7s0VAjwPvmrhrE1jAV4P+Ecg7kgV1DLKXuAjR7J2ZAB3yBNsEZWP
# 3G3UGTp1knjccCyoVqRv/hSvju9QhRmOOMsKRxGOyOFF0nKNQV4k4tl5du4/ZfSi
# tnc04d+mLLH7/J2lMvTvh6gR+Q4U8Izcdfn1U7CrgTIEnXjGjr5eoHhmPIKLNLqk
# DMmhOXsEXF8ZQQsmezdNnPSoaiXEmStZwh0iDAPCu2Celi7rZX7XbhmUUHG5whFY
# XUS52GfbK8ghNSqKuUjI4lDJxQ2x+CXrX6G4hrJvmwnPxWqjePvZrFn+onPnu7QX
# VEaC9ZRz1Yx5H9um4EpE5xOW6BRUhiFptH/UFsoTOBVoUxJTYq8A690+7KAeWsFD
# CcoH+wClQUPE9FhppwZLzlWA7YDyzsQ3IXng1FcLBVED2RpSM3ZOSzcF5zWG0a/0
# Nqip3Roh8/PEKE7NGCb4S4Bf5lyeAE9ZmBeiLNcDf37VQmDTdf9wb/rTDj+B2kOr
# V1GPIv45dtrKzCTl+6nZAC2tXxHOQT+y5tozxZ7tQFL3+SQXosRY9VLI6zsLWimu
# 0RTvqQ8DhaGCAyYwggMiBgkqhkiG9w0BCQYxggMTMIIDDwIBATB9MGkxCzAJBgNV
...
# CSqGSIb3DQEJBDEiBCCLvPJC1dTiuY2jpbrdEwEATQbI/OnBVhKcEiD8WU7OfjAN
# BgkqhkiG9w0BAQEFAASCAgCdo8f9336P1dHy9TWpbHkBAtnqPHkIgH64D5QJQewh
# CI0qkBH/E4DpS9O82TGKr+SFpipDlmxqp4GmMqm+FX8zV+W7w2bnHVHmIZOxanJB
# HVzcNqw95irDdcUFeqlZJiSolfoUz7uFA0xyOhUcddnpux5DpJMdY3IJW+nK/wRr
# CMWSn8G8DSSgSZn5nZUbr9/I7t703Mtw3+fEuhV/m/hMJB8QJLSfkMD6rJzTzlCW
# e7FU5cpEXewVJx4yEl9+2FU+iUIE9fucnMNFCeDmkQmoa4XUwse88vo670N9Hp5e
# XwWvKmX6A4+B+IwLxWZ/vZhdPr5u5M4oDc6OoDCE7q+/VFWI/dHq7UwSMQrR85aJ
# QQrTzf+qtIGdqdlvwaOsoYDSt8+q8oFY8RHSl7d/dASM+uaYxkn7ep/zVGpD15Eu
# 0NwbZqW6MztG7RP5SuprXWx9X/1KcfKKsIzh6O02ukCQvzx/amundpPbkxRWhoFh
# hR+D1j3kfyQMcmN9g0OVr0ziqjOKW1u6p694fvlqxjxx5PwyyEXIE9yld21rR0xp
# Kj/UbQe40ofVt7LL75LH7D3ou5iMs9LBLnRuju7OdxGHSKif5g8yoXWHwC85//un
# L7mCLR+pxXJkHp4VZnce01JFGwcNBj+ooDcPmeU+k84h93d9WhB05jzOOoW4BGMN
# wg==
