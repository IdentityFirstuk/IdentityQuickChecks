...
<#
.SYNOPSIS
Create a compatibility shim and optional module folder so consumers can import `IdentityHealthCheck`
while preserving the existing `IdentityFirst.QuickChecks` layout.

This is a safe, reversible helper that does not delete or move existing files by default.

.PARAMETER Perform
If specified, perform the changes. Otherwise the script will show a dry-run preview.

.PARAMETER SourceModuleFolder
Path (relative to repo root) to the existing module folder. Default: `Module\IdentityFirst.QuickChecks`.

.PARAMETER TargetModuleFolder
Path (relative to repo root) for the compatibility shim. Default: `Module\IdentityHealthCheck`.

Examples:
    # Dry-run (default)
    .\.scripts\upgrade_to_identityhealthcheck.ps1

    # Perform the upgrade shim creation
    .\.scripts\upgrade_to_identityhealthcheck.ps1 -Perform
#>
[CmdletBinding()]
param(
    [switch]$Perform,
    [string]$SourceModuleFolder = 'Module\IdentityFirst.QuickChecks',
    [string]$TargetModuleFolder = 'Module\IdentityHealthCheck'
)

function Write-Info($msg){ Write-Host "[INFO] $msg" }

$repoRoot = Get-Location
$src = Join-Path $repoRoot.Path $SourceModuleFolder
$tgt = Join-Path $repoRoot.Path $TargetModuleFolder

if (-not (Test-Path $src)){
    Write-Error "Source module folder not found: $SourceModuleFolder"
    exit 1
}

Write-Info "Source found: $SourceModuleFolder"
Write-Info "Target shim location: $TargetModuleFolder"

# Determine module version from source manifest if present
$srcManifest = Join-Path $src 'IdentityFirst.QuickChecks.psd1'
$version = '1.0.0'
if (Test-Path $srcManifest){
    try{
        $m = Import-PowerShellDataFile -Path $srcManifest
        if ($m.ModuleVersion){ $version = $m.ModuleVersion }
    } catch { }
}

if (-not $Perform){
    Write-Info "Dry-run: the following actions would be taken when run with -Perform:`n";
    Write-Host " - Create folder: $TargetModuleFolder"
    Write-Host " - Create manifest: $TargetModuleFolder\IdentityHealthCheck.psd1 (ModuleVersion: $version)"
    Write-Host " - Create module file: $TargetModuleFolder\IdentityHealthCheck.psm1 (compat shim)"
    Write-Host "No files were changed. Re-run with -Perform to create the compatibility shim."
    return 0
}
...
# Create module manifest that nests the original module so its exported commands are re-exported
$guid = [guid]::NewGuid().ToString()
$manifestPath = Join-Path $tgt 'IdentityHealthCheck.psd1'
Write-Info "Writing manifest: $manifestPath"
New-ModuleManifest -Path $manifestPath `
    -RootModule 'IdentityHealthCheck.psm1' `
    -ModuleVersion $version `
    -GUID $guid `
    -Author 'IdentityFirst' `
    -CompanyName 'IdentityFirst' `
    -Copyright '(c) IdentityFirst' `
    -Description 'Compatibility shim module that re-exports IdentityFirst.QuickChecks as IdentityHealthCheck' `
    -NestedModules @('..\\IdentityFirst.QuickChecks\\IdentityFirst.QuickChecks.psm1') `
    -FunctionsToExport @('*') `
    -CmdletsToExport @()

# Create a tiny module file (no-op): nested module is listed in the manifest
$psm1Path = Join-Path $tgt 'IdentityHealthCheck.psm1'
$psm1 = @'
<#
.SYNOPSIS
Compatibility shim module to expose `IdentityFirst.QuickChecks` under the `IdentityHealthCheck` module name.

This file is intentionally minimal. The manifest nests the original module so exported
commands are available when this module is imported.
#>

# Nothing to do here; manifest's NestedModules handles loading and exporting.
'@

Set-Content -Path $psm1Path -Value $psm1 -Force

Write-Info "Compatibility shim created at: $TargetModuleFolder"
Write-Info "Verify by importing: Import-Module $TargetModuleFolder\IdentityHealthCheck.psd1; Get-Command -Module IdentityHealthCheck"

Write-Output "Done"
# SIG # Begin signature block
# MIIf3QYJKoZIhvcNAQcCoIIfzjCCH8oCAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCH/VRkcl8H5GbW
# GZanKWhBC6WpsBWWR5dEWiYiVDYPO6CCGNwwggWNMIIEdaADAgECAhAOmxiO+dAt
...
#