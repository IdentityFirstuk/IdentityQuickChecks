...
# ============================================================================
param(
    [string]$PssaReport = '.\pssa-report.json',
    [string]$Summary = '.scripts\apply_safe_fixes_summary.json',
    [string]$OutMd = '.\IFQC-FixReport.md'
)

function Load-Json([string]$path){ if (-not (Test-Path $path)) { return $null } ; Get-Content -Raw -LiteralPath $path | ConvertFrom-Json }

$pssa = Load-Json $PssaReport
$sum = Load-Json $Summary

$lines = @()
$lines += "# IFQC Fix Report"
$lines += "Generated: $(Get-Date -Format o)"
$lines += ""

if ($sum -ne $null) {
    $lines += "## Safe fixer summary"
    $lines += "- Applied: $($sum.Applied)"
    $lines += "- Files flagged: $($sum.Modified.Count)"
    foreach ($m in $sum.Modified) { $lines += "  - $m" }
    $lines += ""
}

if ($pssa -ne $null) {
    $lines += "## Top PSScriptAnalyzer findings (sample)"
    $findings = @()
    if ($pssa.Count -gt 0) { $findings = $pssa } else { $findings = @() }
    $sample = $findings | Select-Object -First 20
    foreach ($f in $sample) {
        $lines += "- [$($f.RuleName)] $($f.File) -- $($f.Message) (Line $($f.Line))"
    }
    $lines += ""
} else {
    $lines += "No `pssa-report.json` found. Run `.scripts/run_psa_and_tests.ps1` to generate analyzer output."
}

$lines += "## Next recommended manual actions"
$lines += "- Inspect the listed files and backups (.bak.*) for unintended edits."
$lines += "- Manually convert sensitive password params to `SecureString`/`PSCredential`."
$lines += "- Triage `PSAvoidStateChangingInReadOnlyTools` entries and add `-ReadOnly` guards or ShouldProcess around state-changing code."

$lines | Out-File -FilePath $OutMd -Encoding UTF8 -Force
Write-Host "Report written to $OutMd"
# SIG # Begin signature block
# MIIf3QYJKoZIhvcNAQcCoIIfzjCCH8oCAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA6R7fxHTHa/HHH
# zimZhBoMO818yG58acZysyLO0AFywKCCGNwwggWNMIIEdaADAgECAhAOmxiO+dAt
...
# AgEVMC8GCSqGSIb3DQEJBDEiBCDWtJGJat1qQ3ixR0ov47WbZfplLpigUW0HhikB
# Kb81bTANBgkqhkiG9w0BAQEFAASCAgCek4PNlNISzTuIDiwnWH0b9BFBckCqzfDa
# RORwVWnHdXXryB/dftDJ/CnW/Aeg6VY1vkJjBvx6f6U7RJ7xbXkqZ3ImgEgDNYPf
# 9gLXyAPAyFbrxEUjoNClpXwDLdwMXOxxfcwY5HB99sBDgsN7AEnfF1MGBg2p2BJe
# DEhHNakyD+7BzbHEm0m2IehlH1WLuEhrJaPSMHAcvdteNBctPC7ZvWjgHTGSPff/
